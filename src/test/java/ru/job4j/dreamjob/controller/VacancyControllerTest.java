...
class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyGetByIdThenGetOne() {
    ...
    @Test
    public void whenVacancyNotFoundThenReturn404Page() {
    ...
    public void whenUpdateVacancyWithFileThenSameDataAndRedirectToVacanciesPa7ge() throws Exception {
        ...
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(vacancyArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(vacancy.getVisible());
    ...
    public void whenUpdateVacancyWithInvalidIdThenReturn404Page() {
        ...
        var viewName = vacancyController.update(vacancy, testFile, model);

        assertEquals("errors/404", viewName);

        var message = model.asMap().get("message");
        assertEquals("Вакансия с указанным идентификатором не найдена", message);
        ...
        assertEquals(vacancy, vacancyCaptor.getValue());
    }
    ...
    public void whenDeleteVacancySuccessfullyThenRedirectToVacanciesPage() {
        ...
        var viewName = vacancyController.delete(model, vacancyId);

        assertEquals("redirect:/vacancies", viewName);

        verify(vacancyService).deleteById(vacancyId);
    }
