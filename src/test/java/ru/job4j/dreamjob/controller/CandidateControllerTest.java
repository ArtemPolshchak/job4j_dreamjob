...
class CandidateControllerTest {
    ...
    @Test
    public void whenRequestCandidateGetByIdThenGetOne() {
    ...
    @Test
    public void whenCandidateNotFoundThenReturn404Page() {
    ...
    @Test
    public void whenUpdateVacancyWithFileThenSameDataAndRedirectToVacanciesPa7ge() throws Exception {
        var candidate = new Candidate(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        ...
        when(candidateService.update(candidateArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(candidate.getVisible());
    ...
    public void whenUpdateVacancyThenRedirectToVacanciesPage() throws Exception {
        ...
        when(candidateService.update(candidateArgumentCaptor.capture(),
                fileDtoArgumentCaptor.capture())).thenReturn(candidate.getVisible());
    ...
    @Test
    public void whenUpdateVacancyWithInvalidIdThenReturn404Page() {
        var candidate = new Candidate(1, "test1", "desc1", now(), true, 1, 2);
        var testFile = new MockMultipartFile("testFile.img", new byte[]{1, 2, 3, 4});

        when(candidateService.update(any(Candidate.class), any(FileDto.class))).thenReturn(false);

        var model = new ConcurrentModel();

        var viewName = candidateController.update(candidate, testFile, model);

        assertEquals("errors/404", viewName);

        var message = model.asMap().get("message");
        assertEquals("Кандидат с указанным идентификатором не найден", message);
        ...
        assertEquals(candidate, candidateCaptor.getValue());
    }

    @Test
    public void whenDeleteVacancySuccessfullyThenRedirectToVacanciesPage() {
        int candidateId = 1;
        ...
        var viewName = candidateController.delete(model, candidateId);

        assertEquals("redirect:/candidates", viewName);

        verify(candidateService).deleteById(candidateId);
    }

    @Test
    public void whenDeleteVacancyFailsThenReturn404Page() {
        int candidateId = 1;
